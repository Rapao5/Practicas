...
namespace Ejercicio1
{
	...
	class Program
	{
		...
		{
            Investigador persona = new Investigador(1, "Raúl Tower", "Defensa en CSS");
            Investigador persona2 = new Investigador(2, "Jose Profe", "Enseñanza avanzada");
            Investigador persona3 = new Investigador(3, "Alex God", "Jefe de programación");
            ...
            Console.WriteLine("LISTA INVESTIGADORES:");
            foreach (var investigador in investigadores)
            {
                Console.WriteLine(investigador.GenerateReport());
            }
            
            var investigadoresFiltrados = investigadores.Where(investigador => investigador.especialidad == "Enseñanza avanzada");

            Console.WriteLine("LISTA INVESTIGADORES CON ENSEÑANZA AVANZADA:");
            foreach (var investigador in investigadoresFiltrados)
            {
                Console.WriteLine(investigador.GenerateReport());
            }

            var investigadoresAlfabetico = investigadores.OrderBy(investigador => investigador.nombre);

            Console.WriteLine("LISTA INVESTIGADORES ORDENADOR ALFABÉTICAMENTE:");
            foreach (var investigador in investigadoresAlfabetico)
            {
                Console.WriteLine(investigador.GenerateReport());
            }

            var investigadoresNombre = investigadores.Select(investigador => investigador.nombre);

            Console.WriteLine("LISTA DE NOMBRES DE LOS INVESTIGADORES:");
            foreach (var investigador in investigadoresNombre)
            {
